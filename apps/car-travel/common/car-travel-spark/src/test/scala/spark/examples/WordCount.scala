...
object WordCount {
  def main(args: Array[String]): Unit = {
    ...
    sc.setCheckpointDir("/sparkapp/tmp");//add by MiAN

    //3.使用spark上下文的api接口textFile读取文件形成spark数据处理模型RDD
    val linesRDD: RDD[String] = sc.textFile("D:\\gitHome\\JavaBase\\apps\\car-travel\\common\\car-travel-spark\\src\\test\\scala\\words")
